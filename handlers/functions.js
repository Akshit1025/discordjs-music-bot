...
function createBar(player) {
  let { size, line, slider, leftindicator, rightindicator, style } =
  ...
  if (style == "simple") {
    ...
    try {
      if (!player.queue.current)
        return `**${emoji.msg.progress_bar.empty_left}${
          emoji.msg.progress_bar.filledframe
        }${emoji.msg.progress_bar.emptyframe.repeat(size - 1)}${
          emoji.msg.progress_bar.empty_right
        }**\n**00:00:00 / 00:00:00**`;
      let current =
        player.queue.current.duration !== 0
          ? player.position
      ...
      if (leftside < 1)
        bar =
          String(emoji.msg.progress_bar.empty_left) +
          String(emoji.msg.progress_bar.emptyframe).repeat(rightside) +
          String(emoji.msg.progress_bar.empty_right);
      else
        bar =
          String(emoji.msg.progress_bar.filled_left) +
          String(emoji.msg.progress_bar.filledframe).repeat(leftside) +
          String(emoji.msg.progress_bar.emptyframe).repeat(rightside) +
          String(
            size - rightside !== 1
              ? emoji.msg.progress_bar.empty_right
              : emoji.msg.progress_bar.filled_right
          );
      return `**${bar}**\n**${
        new Date(player.position).toISOString().substr(11, 8) +
        " / " +
...
function stations(client, prefix, message) {
  ...
  try {
    ...
    const stationsembed4 = new MessageEmbed()
      ...
      .setFooter(client.getFooter(es))
      .setTitle(
        "CUSTOM REQUESTS | Pick your Station, by typing in the right `INDEX` Number!"
      );

    let beforeindex = 1;
    let REYFM = "";
    for (let i = 0; i < radios.REYFM.length; i++) {
      REYFM += `**${i + beforeindex}** [${radios.REYFM[i]
        .split(" ")[0]
        .replace("-", " ")
        .substr(0, 16)}](${radios.REYFM[i].split(" ")[1]})\n`;
    }
    beforeindex += radios.REYFM.length;
    let ILOVERADIO = "";
    for (let i = 0; i < radios.ILOVERADIO.length; i++) {
      ILOVERADIO += `**${i + beforeindex}** [${radios.ILOVERADIO[i]
        .split(" ")[0]
        .replace("-", " ")
        .substr(0, 16)}](${radios.ILOVERADIO[i].split(" ")[1]})\n`;
    }
    ...
    for (let i = 0; i < radios.EU.United_Kingdom.length; i++) {
      United_Kingdom += `**${i + beforeindex}** [${radios.EU.United_Kingdom[i]
        .split(" ")[0]
        .replace("-", " ")
        .substr(0, 16)}](${radios.EU.United_Kingdom[i].split(" ")[1]})\n`;
    }
    ...
    for (let i = 0; i < radios.EU.Austria.length; i++) {
      austria += `**${i + beforeindex}** [${radios.EU.Austria[i]
        .split(" ")[0]
        .replace("-", " ")
        .substr(0, 16)}](${radios.EU.Austria[i].split(" ")[1]})\n`;
    }
    ...
    for (let i = 0; i < radios.EU.Belgium.length; i++) {
      Belgium += `**${i + beforeindex}** [${radios.EU.Belgium[i]
        .split(" ")[0]
        .replace("-", " ")
        .substr(0, 16)}](${radios.EU.Belgium[i].split(" ")[1]})\n`;
    }
    ...
    for (let i = 0; i < radios.EU.Bosnia.length; i++) {
      Bosnia += `**${i + beforeindex}** [${radios.EU.Bosnia[i]
        .split(" ")[0]
        .replace("-", " ")
        .substr(0, 16)}](${radios.EU.Bosnia[i].split(" ")[1]})\n`;
    }
    ...
    for (let i = 0; i < radios.EU.Czech.length; i++) {
      Czech += `**${i + beforeindex}** [${radios.EU.Czech[i]
        .split(" ")[0]
        .replace("-", " ")
        .substr(0, 16)}](${radios.EU.Czech[i].split(" ")[1]})\n`;
    }
    ...
    for (let i = 0; i < radios.EU.Denmark.length; i++) {
      Denmark += `**${i + beforeindex}** [${radios.EU.Denmark[i]
        .split(" ")[0]
        .replace("-", " ")
        .substr(0, 16)}](${radios.EU.Denmark[i].split(" ")[1]})\n`;
    }
    ...
    for (let i = 0; i < radios.EU.Germany.length; i++) {
      germany += `**${i + beforeindex}** [${radios.EU.Germany[i]
        .split(" ")[0]
        .replace("-", " ")
        .substr(0, 16)}](${radios.EU.Germany[i].split(" ")[1]})\n`;
    }
    ...
    for (let i = 0; i < radios.EU.Hungary.length; i++) {
      Hungary += `**${i + beforeindex}** [${radios.EU.Hungary[i]
        .split(" ")[0]
        .replace("-", " ")
        .substr(0, 16)}](${radios.EU.Hungary[i].split(" ")[1]})\n`;
    }
    ...
    for (let i = 0; i < radios.EU.Ireland.length; i++) {
      Ireland += `**${i + beforeindex}** [${radios.EU.Ireland[i]
        .split(" ")[0]
        .replace("-", " ")
        .substr(0, 16)}](${radios.EU.Ireland[i].split(" ")[1]})\n`;
    }
    ...
    for (let i = 0; i < radios.EU.Italy.length; i++) {
      Italy += `**${i + beforeindex}** [${radios.EU.Italy[i]
        .split(" ")[0]
        .replace("-", " ")
        .substr(0, 16)}](${radios.EU.Italy[i].split(" ")[1]})\n`;
    }
    ...
    for (let i = 0; i < radios.EU.Luxembourg.length; i++) {
      Luxembourg += `**${i + beforeindex}** [${radios.EU.Luxembourg[i]
        .split(" ")[0]
        .replace("-", " ")
        .substr(0, 16)}](${radios.EU.Luxembourg[i].split(" ")[1]})\n`;
    }
    ...
    for (let i = 0; i < radios.EU.Romania.length; i++) {
      Romania += `**${i + beforeindex}** [${radios.EU.Romania[i]
        .split(" ")[0]
        .replace("-", " ")
        .substr(0, 16)}](${radios.EU.Romania[i].split(" ")[1]})\n`;
    }
    ...
    for (let i = 0; i < radios.EU.Serbia.length; i++) {
      Serbia += `**${i + beforeindex}** [${radios.EU.Serbia[i]
        .split(" ")[0]
        .replace("-", " ")
        .substr(0, 16)}](${radios.EU.Serbia[i].split(" ")[1]})\n`;
    }
    ...
    for (let i = 0; i < radios.EU.Spain.length; i++) {
      Spain += `**${i + beforeindex}** [${radios.EU.Spain[i]
        .split(" ")[0]
        .replace("-", " ")
        .substr(0, 16)}](${radios.EU.Spain[i].split(" ")[1]})\n`;
    }
    ...
    for (let i = 0; i < radios.EU.Sweden.length; i++) {
      Sweden += `**${i + beforeindex}** [${radios.EU.Sweden[i]
        .split(" ")[0]
        .replace("-", " ")
        .substr(0, 16)}](${radios.EU.Sweden[i].split(" ")[1]})\n`;
    }
    ...
    for (let i = 0; i < radios.EU.TURKEY.length; i++) {
      TURKEY += `**${i + beforeindex}** [${radios.EU.TURKEY[i]
        .split(" ")[0]
        .replace("-", " ")
        .substr(0, 16)}](${radios.EU.TURKEY[i].split(" ")[1]})\n`;
    }
    beforeindex += radios.EU.TURKEY.length;
    stationsembed3.addField("🇹🇷 TURKEY", `>>> ${TURKEY}`, true);
    let Ukraine = "";
    for (let i = 0; i < radios.EU.Ukraine.length; i++) {
      Ukraine += `**${i + beforeindex}** [${radios.EU.Ukraine[i]
        .split(" ")[0]
        .replace("-", " ")
        .substr(0, 16)}](${radios.EU.Ukraine[i].split(" ")[1]})\n`;
    }
    ...
    for (let i = 0; i < radios.OTHERS.request.length; i++) {
      requests += `**${i + beforeindex}** [${radios.OTHERS.request[i]
        .split(" ")[0]
        .replace("-", " ")
        .substr(0, 20)}](${radios.OTHERS.request[i].split(" ")[1]})\n`;
  ...
  try {
    ...
    if (
      ...
      let embed = new MessageEmbed()
        .setTitle(
          eval(
            client.la[ls]["handlers"]["functionsjs"]["functions"]["variable7"]
          )
        )
        .setDescription(
          settings.LeaveOnEmpty_Queue.enabled && type != "skip"
            ? `I'll leave the Channel: \`${
                client.channels.cache.get(player.voiceChannel).name
              }\` in: \`${ms(config.settings.LeaveOnEmpty_Queue.time_delay, {
                long: true,
              })}\`, If the Queue stays Empty! `
            : eval(
                client.la[ls]["handlers"]["functionsjs"]["functions"][
                  "variable9"
                ]
              )
        )
    ...
    if (guildid) {
      ...
      client.settings.ensure(guildid, {
        ...
        embed: {
          ...
          footertext: client.guilds.cache.get(guildid)
