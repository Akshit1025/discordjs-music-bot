...
module.exports = (client) => {
  ...
  client.updateMusicSystem = async (player, leave = false) => {
    if (
      ...
      if (
        ...
        return client.logger("Music System - Missing Permissions");
      //try to get the channel
      let message = channel.messages.cache.get(messageId);
      if (!message)
        message =
          (await channel.messages.fetch(messageId).catch(() => {})) || false;
      if (!message) return client.logger("Music System - Message not found!");
      //edit the message so that it's right!
      var data =
        require(`${process.cwd()}/handlers/erela_events/musicsystem`).generateQueueEmbed(
          client,
          player.guild,
          leave
        );
  ...
  client.editLastPruningMessage = async (
    player,
    footertext = "\n⛔️ SONG ENDED!"
  ) => {
    ...
    let guild = client.guilds.cache.get(player.guild);
    if (!guild)
      return client.logger("Editing the Last Message - Guild not found!");
    //try to get the channel
    let channel = guild.channels.cache.get(player.textChannel);
    if (!channel)
      channel =
        (await guild.channels.fetch(player.textChannel).catch(() => {})) ||
        false;
    if (!channel)
      return client.logger("Editing the Last Message - Channel not found");
    if (
      !channel
        .permissionsFor(channel.guild.me)
        .has(Permissions.FLAGS.SEND_MESSAGES)
    )
      return client.logger("Editing the Last Message - Missing Permissions");
    ...
    if (!message)
      message =
        (await channel.messages
          ...
          .catch(() => {})) || false;
    if (!message)
      return client.logger("Editing the Last Message - Message not found!");
    if (!message.embeds || !message.embeds[0])
      return client.logger("Editing the Last Message - Embeds got removed!");
    ...
    if (settings.deleteMessagesBeforeTheLastSongPlayedMessages) {
      ...
      let beforemessage = channel.messages.cache.get(
        player.get("beforemessage")
      );
      if (!beforemessage)
        message =
          (await channel.messages
            .fetch(player.get("beforemessage"))
            .catch(() => {})) || false;
      if (!beforemessage)
        return client.logger(
          "Editing the Last Message - Before - Message not found!"
        );
      //if not able to
      if (!beforemessage)
        return client.logger(
          "Editing the Last Message - Before - Message already deleted"
        );
      if (!beforemessage.deletable)
        return client.logger(
          "Editing the Last Message - Before - Message not delete able"
        );
      //delete the message
  ...
  client.getFooter = (es, stringurl = null) => {
    ...
    let embedData = {};
    if (typeof es !== "object")
      embedData = { footertext: es, footericon: stringurl };
    else embedData = es;
    ...
    if (!iconURL.startsWith("https://") && !iconURL.startsWith("http://"))
      iconURL = client.user.displayAvatarURL();
    if (
      ![".png", ".jpg", ".wpeg", ".webm", ".gif"].some((d) =>
        iconURL.toLowerCase().endsWith(d)
      )
    )
      iconURL = client.user.displayAvatarURL();
