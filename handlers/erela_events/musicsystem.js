...
module.exports = (client) => {
  client.on("interactionCreate", async (interaction) => {
    ...
    if (interaction.isSelectMenu()) {
      ...
      if (interaction.values[0]) {
        ...
        //pop
        if (interaction.values[0].toLowerCase().startsWith("p"))
          link =
            "https://www.youtube.com/playlist?list=PLMC9KNkIncKtPzgY-5rmhvj7fax8fdxoj";
        //default
        if (interaction.values[0].toLowerCase().startsWith("d"))
          link =
            "https://www.youtube.com/playlist?list=PLMC9KNkIncKtPzgY-5rmhvj7fax8fdxoj";
        //remixes from Magic Release
        if (interaction.values[0].toLowerCase().startsWith("re"))
          link =
            "https://www.youtube.com/watch?v=NX7BqdQ1KeU&list=PLYUn4YaogdahwfEkuu5V14gYtTqODx7R2";
        //rock
        if (interaction.values[0].toLowerCase().startsWith("ro"))
          link = "https://open.spotify.com/playlist/37i9dQZF1DWXRqgorJj26U";
        //oldgaming
        if (interaction.values[0].toLowerCase().startsWith("o"))
          link =
        ...
        embeds: [
          new MessageEmbed()
            ...
            .setAuthor(
              client.getAuthor(
                `Loading '${
                  interaction.values[0] ? interaction.values[0] : "Default"
                }' Music Mix`,
            ...
            .setTitle(
              eval(client.la[ls]["cmds"]["music"]["playmusicmix"]["variable1"])
            )
            .setDescription(
              eval(client.la[ls]["cmds"]["music"]["playmusicmix"]["variable2"])
            )
            .addField(
              eval(
                client.la[ls]["cmds"]["music"]["playmusicmix"]["variablex_3"]
              ),
  ...
  client.on("messageCreate", async (message) => {
    ...
    if (!channel)
      return message
        .reply(
          "<:no:833101993668771842> **Please join a Voice Channel first!**"
        )
        .then((msg) => {
          setTimeout(() => {
            try {
    ...
    if (player && channel.id !== player.voiceChannel)
      return message
        .reply(
          `<:no:833101993668771842> **Please join __my__ Voice Channel first! <#${player.voiceChannel}>**`
        )
        .then((msg) => {
          setTimeout(() => {
            try {
...
function generateQueueEmbed(client, guildId, leave) {
    ...
    new MessageEmbed()
      ...
      .setImage(
        guild.banner
          ...
          : "https://i.postimg.cc/3xBy0G3c/ghost-music.png"
      )
      .setTitle(
        `Start Listening to Music, by connecting to a Voice Channel and sending either the **SONG LINK** or **SONG NAME** in this Channel!`
      )
      .setDescription(
        `> *I support <:Youtube:840260133686870036> Youtube, <:Spotify:846090652231663647> Spotify, <:soundcloud:825095625884434462> Soundcloud and direct MP3 Links!*`
      ),
  ];
  ...
  if (!leave && player && player.queue && player.queue.current) {
    embeds[1]
      ...
      .addField(
        ...
        `\`${format(player.queue.current.duration).split(" | ")[0]}\` | \`${
          format(player.queue.current.duration).split(" | ")[1]
        }\``,
        true
      )
      .addField(
    ...
    embeds[0] = new MessageEmbed()
      .setTitle(
        `📃 Queue of __${guild.name}__  -  [ ${player.queue.length} Tracks ]`
      )
      .setColor(es.color)
      .setDescription(
        String(
  ...
  //now we add the components!
  var components = [
    new MessageActionRow().addComponents([musicmixMenu]),
