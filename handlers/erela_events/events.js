...
module.exports = (client) => {
    ...
    for (const gId of guilds) {
      try {
        ...
        if (!guild) {
          ...
          client.logger(
            `Autoresume`.brightCyan + ` - Bot got Kicked out of the Guild`
          );
          continue;
        }
        ...
        if (!textChannel) {
          client.autoresume.delete(gId);
          client.logger(
            `Autoresume`.brightCyan + ` - Text Channel got deleted`
          );
          continue;
        }
        ...
        if (data.current && data.current.identifier) {
          const buildTrack = async (data) => {
            return data.track && data.identifier
              ? TrackUtils.build(
                  ...
                  data.requester
                    ? client.users.cache.get(data.requester) ||
                        (await client.users
          ...
          if (data.queue.length)
            for (let track of data.queue)
              player.queue.add(await buildTrack(track));
        ...
        client.logger(
          `Autoresume`.brightCyan +
            ` - Added ${player.queue.length} Tracks on the QUEUE and started playing ${player.queue.current.title} in ${guild.name}`
        );
        ...
        switch (data.filter) {
          case "💯 Vibrato":
            {
              require("../../commands/👀 Filter/vibrato").run(
                client,
                {
                  guild: {
                    id: player.guild,
          ...
          case "💢 Vibrate":
            {
              require("../../commands/👀 Filter/vibrate").run(
                client,
                {
                  guild: {
                    id: player.guild,
          ...
          case "🏮 Tremolo":
            {
              require("../../commands/👀 Filter/tremolo").run(
                client,
                {
                  guild: {
                    id: player.guild,
          ...
          case "⏱ Speed":
            {
              require("../../commands/👀 Filter/speed").run(
                client,
                {
                  guild: {
                    id: player.guild,
          ...
          case "⏱ Slowmode":
            {
              require("../../commands/👀 Filter/slowmo").run(
                client,
                {
                  guild: {
                    id: player.guild,
          ...
          case "📉 Rate":
            {
              require("../../commands/👀 Filter/rate").run(
                client,
                {
                  guild: {
                    id: player.guild,
          ...
          case "📈 Pitch":
            {
              require("../../commands/👀 Filter/pitch").run(
                client,
                {
                  guild: {
                    id: player.guild,
          ...
          case "👻 Nightcore":
            {
              require("../../commands/👀 Filter/nightcore").run(
                client,
                {
                  guild: {
                    id: player.guild,
          ...
          case "👾 Darth Vader":
            {
              require("../../commands/👀 Filter/darthvader").run(
                client,
                {
                  guild: {
                    id: player.guild,
          ...
          case "🐿️ Chipmunk":
            {
              require("../../commands/👀 Filter/chipmunk").run(
                client,
                {
                  guild: {
                    id: player.guild,
          ...
          case "👺 China":
            {
              require("../../commands/👀 Filter/china").run(
                client,
                {
                  guild: {
                    id: player.guild,
          ...
          case "🎚 Low Bass":
            {
              require("../../commands/👀 Filter/bassboost").run(
                client,
                {
                  guild: {
                    id: player.guild,
          ...
          case "🔊 8D":
            {
              require("../../commands/👀 Filter/3d").run(
                client,
                {
                  guild: {
                    id: player.guild,
  ...
  client.manager
    ...
    .on(`playerCreate`, async (player) => {
      ...
      var interval = setInterval(async () => {
        if (
          ...
          client.logger(
            `Music System - Relevant Checker - Checkingfor unrelevant Messages`
          );
          ...
          if (!guild)
            return client.logger(
              `Music System - Relevant Checker - Guild not found!`
            );
          ...
          if (!channel)
            channel =
              (await guild.channels
                .fetch(client.musicsettings.get(player.guild, `channel`))
                .catch(() => {})) || false;
          if (!channel)
            return client.logger(
              `Music System - Relevant Checker - Channel not found!`
            );
          if (
            !channel
            ...
            return client.logger(
              `Music System - Relevant Checker - Missing Permissions`
            );
          //try to get the channel
          ...
          if (messages.filter((m) => m.id != messageId).size > 0) {
            channel
              ...
              .then((messages) =>
                client.logger(
                  `Music System - Relevant Checker - Bulk deleted ${messages.size} messages`
                )
              );
          } else {
            client.logger(
              `Music System - Relevant Checker - No Relevant Messages`
            );
          }
        }
      }, 60000);
      ...
      var autoresumeinterval = setInterval(async () => {
        ...
        if (client.settings.get(pl.guild, `autoresume`)) {
          ...
          if (data.guild != pl.guild)
            client.autoresume.set(pl.guild, pl.guild, `guild`);
          if (data.voiceChannel != pl.voiceChannel)
            client.autoresume.set(pl.guild, pl.voiceChannel, `voiceChannel`);
          if (data.textChannel != pl.textChannel)
            client.autoresume.set(pl.guild, pl.textChannel, `textChannel`);

          if (
            pl.queue &&
            pl.queue.current &&
            (!data.current ||
              data.current.identifier != pl.queue.current.identifier)
          )
            client.autoresume.set(
              pl.guild,
              makeTrack(pl.queue.current),
              `current`
            );
          if (data.volume != pl.volume)
            client.autoresume.set(pl.guild, pl.volume, `volume`);
          if (data.queueRepeat != pl.queueRepeat)
            client.autoresume.set(pl.guild, pl.queueRepeat, `queueRepeat`);
          if (data.trackRepeat != pl.trackRepeat)
            client.autoresume.set(pl.guild, pl.trackRepeat, `trackRepeat`);
          if (data.playing != pl.playing)
            client.autoresume.set(pl.guild, pl.playing, `playing`);
          if (data.position != pl.position)
            client.autoresume.set(pl.guild, pl.position, `position`);
          if (data.eq != eq) client.autoresume.set(pl.guild, eq, `eq`);
          if (data.filter != filter)
            client.autoresume.set(pl.guild, filter, `filter`);
          if (data.filtervalue != filtervalue)
            client.autoresume.set(pl.guild, filtervalue, `filtervalue`);
          if (data.autoplay != autoplay)
            client.autoresume.set(pl.guild, autoplay, `autoplay`);
    ...
    .on(`playerDestroy`, async (player) => {
      ...
      if (client.autoresume.has(player.guild))
        client.autoresume.delete(player.guild);
      ...
      try {
        ...
        if (playercreated.has(player.guild)) {
          ...
          player.set(
            `autoplay`,
            player.get("autoplay") ||
              client.settings.get(player.guild, `defaultap`)
          );
          ...
          if (player.get("autoresume")) {
            ...
            await player.setVolume(
              client.settings.get(player.guild, `defaultvolume`)
            );
          ...
          client.logger(
            `Player Created in ${
              guild ? guild.name : player.guild
            } | Set the - Guild Default Data`
          );
          /*client.logger({
        ...
        if (
          ...
          return client.logger(
            `No PRUNING-Message sent, because Player-TextChannel == Music System Text Channel`
          );
        }
              ...
              return i.reply({
                embeds: [
                  new MessageEmbed()
                    ...
                    .setFooter(client.getFooter(es))
                    .setTitle(
                      `❌ **You are not a DJ and not the Song Requester!**`
                    )
                    .setDescription(
                      `**DJ-ROLES:**\n${check_if_dj(
                        client,
      ...
      try {
        ...
        if (!player.queue || !player.queue.current) {
          ...
          if (player.get(`afk`)) {
            return client.logger(
              `Queue went empty in ${
                client.guilds.cache.get(player.guild)
                  ? client.guilds.cache.get(player.guild).name
                  : player.guild
              }, not leaving, because AFK is enabled!`
            );
          }
