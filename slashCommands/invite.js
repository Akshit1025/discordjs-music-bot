...
module.exports = {
  ...
  options: [
    ...
    //{"String": { name: "ping_amount", description: "How many times do you want to ping?", required: true }}, //to use in the code: interacton.getString("ping_amount")
    {
      User: {
        name: "other_bot",
        description: "Generate an Invite Link for a Different Bot",
        required: false,
      },
      ...
      if (user) {
        if (!user.bot)
          return interaction.reply({
            ephemeral: true,
            content:
              "<:no:833101993668771842> You can't Invite a Normal user! **IT MUST BE A BOT**",
          });
        ...
        interaction.reply({
          ...
          embeds: [
            new MessageEmbed()
              ...
              .setFooter(
                client.getFooter(
                  `${user.username} | powered by milrato.dev`,
                  "https://imgur.com/jPItIw0.gif"
                )
              ),
          ],
        ...
        let button_invite = new MessageButton()
          .setStyle("LINK")
          .setLabel(handlemsg(client.la[ls].cmds.info.invite.buttons.bot))
          .setURL(
        ...
        interaction.reply({
          ...
          embeds: [
            new MessageEmbed()
              ...
              .setDescription(
                `[*Click here for an Invitelink without Slash Commands*](https://discord.com/api/oauth2/authorize?client_id=${client.user.id}&permissions=8&scope=bot)`
              )
              .setURL(
                `https://discord.com/api/oauth2/authorize?client_id=${client.user.id}&permissions=8&scope=bot%20applications.commands`
              )
              .setFooter(
                `${client.user.username} | powered by milrato.dev`,
                "https://imgur.com/jPItIw0.gif"
              ),
          ],
          components: allbuttons,
