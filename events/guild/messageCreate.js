...
module.exports = async (client, message) => {
  try {
    ...
    if (botchannel.toString() !== "") {
      ...
      if (
        !botchannel.includes(message.channel.id) &&
    ...
    if (cmd.length === 0) {
      if (matchedPrefix.includes(client.user.id))
        return message
          .reply({
            embeds: [
              new Discord.MessageEmbed().setColor(es.color).setTitle(
                handlemsg(client.la[ls].common.ping, {
                  prefix: prefix,
                })
              ),
            ],
    ...
    if (command) {
          ...
          return message.reply({
            embeds: [
              new MessageEmbed()
                ...
                .setDescription(
                  handlemsg(client.la[ls].common.premium.description, {
                    prefix: prefix,
                  })
                ),
            ],
      ...
      if (musicData.channel && musicData.channel == message.channel.id) {
        return message
          .reply("❌ **Please use a Command Somewhere else!**")
          .then((msg) => {
            setTimeout(() => {
              try {
                msg.delete().catch(() => {});
            ...
            .reply({
              embeds: [
                new Discord.MessageEmbed()
                  ...
                  .setFooter(client.getFooter(es))
                  .setTitle(
                    handlemsg(client.la[ls].common.unknowncmd.title, {
                      prefix: prefix,
                    })
                  )
                  .setDescription(
                    handlemsg(client.la[ls].common.unknowncmd.description, {
                      prefix: prefix,
                    })
                  ),
              ],
                  ...
                  .reply({
                    embeds: [
                      new MessageEmbed()
                        .setColor(es.wrongcolor)
                        .setTitle(
                          "❌ There is no current Queue / Song Playing!"
                        ),
                    ],
                  })
                  .catch(() => {});
                ...
                .setDescription(
                  `\`\`\`${
                    e.message
                      ? e.message
                      : e.stack
                      ? String(e.stack).grey.substr(0, 2000)
                      : String(e).grey.substr(0, 2000)
                  }\`\`\``
                ),
            ],
          })
