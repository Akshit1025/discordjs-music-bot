...
module.exports = async (client, oS, nS) => {
  ...
  if (
    ...
    if (
      nS.guild.me.permissions.has(Permissions.FLAGS.SPEAK) ||
      (nS.channel &&
        nS.channel.permissionsFor(nS.guild.me).has(Permissions.FLAGS.SPEAK))
    ) {
    ...
    if (player && oS.channelId == player.voiceChannel) {
      ...
      if (
        !(
          ...
          (oS.streaming && !nS.streaming) ||
          (!oS.serverMute && nS.serverMute && !nS.serverDeaf && !nS.selfDeaf) ||
          (oS.serverMute && !nS.serverMute && !nS.serverDeaf && !nS.selfDeaf) ||
          ...
          setTimeout(async () => {
            try {
              ...
              if (
                ...
                player.destroy();
                client.logger(
                  `Destroyed the Player in ${
                    nS.guild && nS.guild.name ? nS.guild.name : player.guild
                  }, because the Channel got no listeners`
                );
              } else {
   ...
   */
  if (
    nS.id === client.user.id &&
    nS.channelId != oS.channelId &&
    !nS.guild.me.voice.deaf
  ) {
    if (
