...
module.exports = async (client, interaction) => {
      ...
      if (command.parameters) {
        if (command.parameters.type == "music") {
          ...
          if (
            ...
            !command.parameters.notsamechannel
          ) {
            return interaction.reply({
              ephemeral: true,
              embeds: [
                new Discord.MessageEmbed()
                  .setColor(es.wrongcolor)
                  .setFooter(client.getFooter(es))
                  .setTitle(client.la[ls].common.wrong_vc).setDescription(`
                    Channel: < #$ {
                      player.voiceChannel
                    } > `),
              ],
  ...
  if (
    ...
    interaction.customId.includes("PREMIUM-")
  ) {
    ...
    if (!guild)
      return interaction.reply("❌ **I got kicked out of that Guild**");
    let requester = guild.members.cache.get(requesterId);
    if (!requester)
      requester =
        (await guild.members.fetch(requesterId).catch(() => {})) || false;
    if (requester) {
      requester
        .send(
          `${
            interaction.customId == "PREMIUM-ACCEPT"
              ? `✅ **Your Requested for: \`${guild.name}\` got accepted!**`
              : `❌ **Your Requested for: \`${guild.name}\` got declined!**`
          }`
        )
        .catch(() => {});
    }
    if (
      ...
      guild
        .fetchOwner((owner) => {
          owner
            .send(
              `✅ ** Your Guild: \`${guild.name}\` got accepted for PREMIUM!**`
            )
            .catch(() => {});
        })
        .catch(() => {});
    ...
    if (interaction.customId == "PREMIUM-ACCEPT") {
      if (client.premium.get("global", "guilds").includes(guild.id)) {
        interaction.update({
          embeds: [
            interaction.message.embeds[0].setTitle(
              `✅ Guild is already a PREMIUM Member!`
            ),
          ],
          components: [],
        });
        ...
        interaction.update({
          embeds: [
            interaction.message.embeds[0].setTitle(`✅ Accepted the Guild!`),
          ],
          components: [],
        });
      }
      ...
      if (client.premium.get("global", "guilds").includes(guild.id)) {
        ...
        interaction.update({
          embeds: [
            interaction.message.embeds[0].setTitle(`❌ Denied the Guild!`),
          ],
          components: [],
        });
      }
