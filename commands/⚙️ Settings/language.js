...
module.exports = {
  ...
  description:
    "Enable + Change the maximum Percent of UPPERCASE (caps) inside of a Message",
  ...
  run: async (client, message, args, cmduser, text, prefix, player, es, ls) => {
    ...
    async function first_layer() {
      ...
      let MenuEmbed = new Discord.MessageEmbed()
        ...
        .setDescription(
          eval(client.la[ls]["cmds"]["settings"]["setup-language"]["variable1"])
        );
      let used1 = false;
      //send the menu msg
      ...
      function menuselection(menu) {
        ...
        let menuoptionindex = menuoptions.findIndex(
          (v) => v.value == menu.values[0]
        );
        if (menu.values[0] == "Cancel")
          return menu.reply(
            eval(
              client.la[ls]["cmds"]["settings"]["setup-language"]["variable2"]
            )
          );
        menu.deferUpdate();
        used1 = true;
        ...
        case 0: {
          ...
          let helpmsg = await message.reply({
            ...
            embeds: [
              new Discord.MessageEmbed()
                .setTitle(
                  eval(
                    client.la[ls]["cmds"]["settings"]["setup-language"][
                      "variable3"
                    ]
                  )
                )
          ...
          collector.on("collect", async (b) => {
            if (b.user.id !== message.author.id)
              return b.reply(
                `${emoji.msg.ERROR} **Only the one who typed ${prefix}setup-language is allowed to react!**`,
                true
              );
            if (
