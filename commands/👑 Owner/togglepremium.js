      ...
      return message.channel.send({
        embeds: [
          new MessageEmbed()
            ...
            .setTitle(
              eval(client.la[ls]["cmds"]["owner"]["leaveserver"]["variable1"])
            ),
        ],
      });
    if (client.premium.get("global", "guilds").includes(args[0])) {
      ...
      if (guild) {
        guild
          ...
          .then((owner) => {
            owner
              .send(`❌ Your Guild is no longer a \`PREMIUM-GUILD\``)
              .catch(() => {});
          })
          .catch(() => {});
      }
      return message.reply(
        `✅ **The Guild ${
          guild && guild.name ? guild.name : args[0]
        } is now __no longer__ a \`PREMIUM-GUILD\`**`
      );
    } else {
      ...
      if (guild) {
        guild
          ...
          .then((owner) => {
            owner
              .send(`✅ Your Guild is now a \`PREMIUM-GUILD\``)
              .catch(() => {});
          })
          .catch(() => {});
      }
      return message.reply(
        `✅ **The Guild ${
          guild ? guild.name : args[0]
        } is now a \`PREMIUM-GUILD\`**`
      );
    }
  },
};
