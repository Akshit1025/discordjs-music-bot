    ...
    if (args[0]) {
      ...
      if (!cmd && (!cat || cat == null)) {
        return message.reply({
          embeds: [
            embed.setColor(es.wrongcolor).setDescription(
...
📶 **\`${Math.floor(client.ws.ping)}ms\` Ping**`
    ...
    function allotherembeds_eachcategory(filterdisabled = false) {
      ...
      var embed0 = new MessageEmbed()
        ...
        .addField(
          `<:Discord:787321652345438228> **Server Related Commands**`,
          ">>> " +
            client.commands
              .filter(
        ...
        .addField(
          `<:Bot_Flag:835928340715012137> **Bot Related Commands**`,
          ">>> " +
            client.commands
      ...
      var embed5 = new MessageEmbed()
        .setTitle(
          `[\`${
            client.commands.filter((cmd) => cmd.category === "💰 Premium").size
          }\`] 💰 Premium`
        )
        .setDescription(
          `> *${client.commands
            .filter((cmd) => cmd.category === "💰 Premium")
            .sort((a, b) => a.name.localeCompare(b.name))
            .map((cmd) => `\`${cmd.name}\``)
            .join("︲")}*`
        )
      ...
      var embed8 = new MessageEmbed()
        .setTitle(
          `[\`${
            client.commands.filter((cmd) => cmd.category === "⚙️ Settings").size
