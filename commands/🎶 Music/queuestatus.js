...
module.exports = {
  ...
  run: async (client, message, args, cmduser, text, prefix, player, es, ls) => {
    ...
    embed.setTitle(
      eval(client.la[ls]["cmds"]["music"]["queuestatus"]["variable1"])
    );
    embed.setDescription(
      eval(client.la[ls]["cmds"]["music"]["queuestatus"]["variable2"])
    );
    embed.addField(
      `${emoji.msg.raise_volume} Volume`,
      `\`\`\`${player.volume}%\`\`\``,
      true
    );
    embed.addField(
    ...
    embed.addField(
      `📨 Pruning: `,
      `\`\`\`${
        client.settings.get(message.guild.id, "playmsg")
          ? `✅ Enabled`
          : `❌ Disabled`
      }\`\`\``,
      true
