...
const { format } = require(`${process.cwd()}/handlers/functions`);
      ...
      .send({
        embeds: [
          new MessageEmbed()
            ...
            .addField(
              client.la[ls].cmds.music.grab.field2,
              `\`${player.queue.current.author}\``,
              true
            )
            .addField(
              client.la[ls].cmds.music.grab.field3,
              `\`${player.queue.length} Songs\``,
              true
            )
            .addField(
              client.la[ls].cmds.music.grab.field4,
              `\`${prefix}play ${player.queue.current.uri}\``
            )
            .addField(
              client.la[ls].cmds.music.grab.field5,
              `<#${message.channel.id}>`
            )
            .setFooter(
