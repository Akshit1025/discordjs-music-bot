...
module.exports = {
  ...
  run: async (client, message, args, cmduser, text, prefix, player, es, ls) => {
    ...
    if (
      !args.length ||
      (!client.eqs[args[0].toLowerCase()] && args[0].toLowerCase() != `none`)
    )
      return message.channel.send({
        embeds: [
          new MessageEmbed()
            ...
            .setTitle(
              eval(client.la[ls]["cmds"]["filter"]["equalizer"]["variable1"])
            )
            .setDescription(
              eval(client.la[ls]["cmds"]["filter"]["equalizer"]["variable2"])
            ),
        ],
      });
    ...
    return message.channel.send({
      embeds: [
        new MessageEmbed()
          ...
          .setTitle(
            eval(client.la[ls]["cmds"]["filter"]["equalizer"]["variable3"])
          )
          .setDescription(
            eval(client.la[ls]["cmds"]["filter"]["equalizer"]["variable4"])
          ),
      ],
    });
  },
